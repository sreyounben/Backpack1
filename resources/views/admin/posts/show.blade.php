    ...
    <table aria-hidden="true" class="table table-striped">
        <tbody>
            ...
            <tr>
                ...
                <td class="border-0 font-weight-bold">Categories</td>
                <td class="border-0 ">:</td>
                <td class="border-0">
                    @foreach ($entry->categories as $key => $item)
                        @if(count($entry->categories) == $key+1)
                            {{ $item->title }}
                        @else
                            {{ $item->title }} ,
                        @endif
                ...
                <td class="border-0 font-weight-bold">Photos</td>
                <td class="border-0 ">:</td>
                <td class="border-0">
                    <a href="{{ asset('storage/' . $entry->photos) }}"
                        data-lightbox="lightbox">
                        <img src=" {{ asset('storage/' . $entry->photos) }} " alt="Hell" width="35"/>
                    </a>
                </td>
            </tr>
        </tbody>
